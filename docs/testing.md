...
| `test_music_arrangement.py` | the melody and bass side of musical identity: `MOTIF_SETS`/`BASS_LINES` are playable data (motif slots are degree/route/hold labels, bass slots are in range, every referenced motif exists), each family keeps the **fingerprint that defines it** (the hero set opens on a leap of at least an octave, the lyric set must not, and neither starts where the other does), the chromatic approach tone really is a semitone below the root under every chord, and — through the renderer's own `VoiceLog`, not by reading tables — a cue's tags reach the synth: a hero cue leads with hero material, a tagged bass line produces octave pops and approach tones, those pops are louder than the root they answer, an untagged cue keeps the stock bass note-for-note, and the tagged cues differ from untagged renders in more than a third of their samples | 
