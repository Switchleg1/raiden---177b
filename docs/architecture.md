...
| `instruments.py` | Synthesised instruments rendered per note and cached by `(name, pitch, length)`: `flute` (breath, late vibrato), `violin` (harmonic stack, slow attack), `strings` (three detuned bows), `pluck` (partials that rot at different rates), `bell` (inharmonic FM). Pure Python, sine-LUT based, no mixer dependency; `music.py` asks for one by name. |
